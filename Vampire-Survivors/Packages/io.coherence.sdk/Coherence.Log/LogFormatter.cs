...
namespace Coherence.Log
{
    ...
    public static class LogFormatter
    {
        ...
        public static StringBuilder AppendArgs(StringBuilder logBuilder, ICollection<(string key, object value)> args, bool useTab = true)
            => AppendArgs(logBuilder, args, useTab ? '\t' : ' ');

        public static StringBuilder AppendArgs(StringBuilder logBuilder, ICollection<(string key, object value)> args, char delimiter)
        {
            if (args.Count <= 0)
            {
                return logBuilder;
